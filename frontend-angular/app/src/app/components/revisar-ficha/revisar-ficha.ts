...
export class RevisarFichaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Mario: carga de fichas disponibles desde carpeta local: consulta backend para resolver dinámicamente por prefijo FE_XX_CLAVE.
  private async cargarFichasLocales(): Promise<void> {
    const archivosPorPrefijo = new Map<string, { pdf: string | null; pptx: string | null }>();

    try {
      // Mario: obtiene mapeo dinámico de archivos de fichas estatales del backend.
      const response = await fetch('/api/catalogos/ficha-estatales/', {
        cache: 'no-store',
        headers: this.authHeaders,
      });
      ...
      if (response.ok) {
        const payload = (await response.json()) as {
          ok?: boolean;
          items?: Array<{ prefix?: string; pdf?: string | null; pptx?: string | null }>;
        };

        // Mario: mapea los datos por prefijo para acceso rápido sin fecha fija.
        if (payload?.ok && Array.isArray(payload.items)) {
          for (const item of payload.items) {
            const prefix = String(item?.prefix ?? '').trim().toUpperCase();
            if (!prefix) continue;
            archivosPorPrefijo.set(prefix, {
              pdf: item?.pdf?.trim() || null,
              pptx: item?.pptx?.trim() || null,
            });
          }
        }
      }
    } catch {
      // Mario: fallback, sin bloquear pantalla si falla el endpoint.
    }

    // Mario: construye lista de entidades con sus archivos dinámicos resueltos por prefijo.
    this.entidadesConId = Object.entries(ENTIDAD_POR_CLAVE)
      .map(([clave, nombre]) => {
        const orden = ORDEN_ENTIDAD_POR_CLAVE[clave];
        const prefix = `FE_${orden}_${clave}`;
        const archivos = archivosPorPrefijo.get(prefix);

        return {
          id: orden,
          clave,
          nombre,
          nombre_pdf: archivos?.pdf ?? undefined,
          nombre_ppt: archivos?.pptx ?? undefined,
        };
      })
