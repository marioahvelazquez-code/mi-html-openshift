...
export default function RevisarFicha() {
  ...
  const obtenerBaseObjeto = () => {
    ...
    const baseSinExtension = nombrePdf.replace(/\.pdf$/i, "");
    const baseSinHash = baseSinExtension.replace(/-[A-Za-z0-9]{6,}$/, "");

    return baseSinHash.replace(/\s+/g, "_");
  };
  ...
  const hayCambiosLocales = () =>
    revisiones.some((revision, index) => {
      const revisionBase = revisionesBase[index] || { estatus: "", comentario: "" };
      return (
  ...
  useEffect(() => {
    if (tipoFicha !== "estatal" || !pdfSeleccionado || numPages <= 0) {
      return;
    }

    const intervalo = window.setInterval(() => {
      // Evita sobrescribir cambios locales aún no guardados del usuario actual.
  ...
  }, [tipoFicha, pdfSeleccionado, numPages, revisiones, revisionesBase]);
