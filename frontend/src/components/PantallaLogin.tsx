...
export default function PantallaLogin({ onLoginSuccess }: Props) {
  ...
  const [usuariosPermitidos, setUsuariosPermitidos] = useState<Usuario[]>([]);

  useEffect(() => {
    // #Mario: Inicio carga de usuarios locales para autenticacion temporal.
    const cargarUsuarios = async () => {
      try {
        const archivoUsuarios = new URL("../assets/usuarios.json", import.meta.url).href;
  ...
  return (
    <div className="login-screen">
      ...
      <section className="login-card" aria-label="Autenticacion de acceso">
        ...
        <p className="login-subtitle">Inicia sesion para acceder a revisar solicitudes</p>
